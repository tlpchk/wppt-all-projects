...
$CARDGROUP=<<<EOT
<h2 class="accordion card-group-header rb">{{HEADER}}</h2>
<section class="container card-group panel">
{{CARDS}}
</section>
EOT;
...
<div class="cardsect {{GRID}} ">
	<a href="{{HREF}}">
	<div class="card {{TYPE}} rb">
		<h2 class="rtb ">{{TITLE}}</h2>
...
class CardList{
	...
	public function create(){
		...
		$C = $this->items;
		$P1 = '			<li>{{NAME}}</li>'. "\n";
		$P2 = '<li><a class="rb" href="{{HREF}}">{{NAME}}</a></li>'. "\n";
		foreach ($C as $item){
			if (is_string($item)){
		...
		$card = str_replace(["{{ITEMS}}","{{FORMULA}}"], [join("\n",$X),$this->formula], $card);
		return preg_replace('/^\h*\v+/m', '', $card)."\n";
		}
}
