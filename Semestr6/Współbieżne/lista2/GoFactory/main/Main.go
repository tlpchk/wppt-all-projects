...
func main() {
	...
	var addComputers [ADD_COMPUTERS]*AddComputer
	var mulComputers [MUL_COMPUTERS]*MulComputer
	...
	for i := range addComputers {
		addComputers[i] = &AddComputer{make(chan Task, 1)}
		go goComputer(addComputers[i])
	}

	for i := range mulComputers {
		mulComputers[i] = &MulComputer{make(chan Task, 1)}
		go goComputer(mulComputers[i])
	}
